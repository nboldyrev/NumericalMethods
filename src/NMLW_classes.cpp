...
/*  class Matrix {
    ...
    public:
        Matrix():data(){}
        Matrix(const int c,const int r):rows(r),cols(c) {
            data.resize(cols,std::vector<MyType>(rows,0));
        }
        Matrix(std::initializer_list<std::vector<MyType>> list) : data(list) {
            cols = data.size();
            rows = data[0].size();
        }
        Matrix(const std::vector<MyType>& col):cols(1),rows(col.size()){
            data[0] = col;
        }
        size_t getRows() {
            return this->rows;
        ...
        Matrix get_col(const int index){
            const auto tmp = data[index];
            return Matrix(tmp);
        }

        const Matrix get_col(const int index) const {
            const auto tmp = data[index];
            return Matrix(tmp);
        }

        
        std::vector<MyType> get_row(const int index){
        ...
        std::vector<MyType> set_row(const int index,const Matrix new_row){
            std::vector<MyType> row;
            for(int i = 0; i < this->cols; ++i) {
                data[i][index]=new_row(i,index);
            }
            return row;
        }
        ...
        Matrix addToRow(const size_t rowLhs,const size_t rowRhs, const MyType scalar) {
            ...
            for(size_t i = 0; i < cols; ++i) {
                    result(i,rowLhs)+=scalar*result(i,rowRhs);
            }
            return result;
        }
   ...
   std::pair<MyType, MyType> getMaxPosition(const matrix& m_matrix, const int var) {
    ...
    void matrixToUpperTriangleFormFullChoice(matrix& m_matrix) {
		...
		for (auto i = 0; i < hight-1 ; ++i) {
            ...
            std::cout<<max_col<<" "<<max_row<<std::endl;
			MatrixOperations::swapRows(m_matrix,i, max_row);
            MatrixOperations::swapCols(m_matrix,i, max_col);
            for(auto j = i + 1; j < hight; ++j) {
                auto c = m_matrix[j][i] / m_matrix[i][i];
...
namespace Lab1 {
    ...
    void inverseMatirxDemo() {
        matrix m_matrix;
        DataComputing::readData(fileIn4,m_matrix);
/*         MatrixOperations::matrixToUpperTriangleFormFullChoice(m_matrix);
        DataComputing::writeMatrix(m_matrix); */
 ...
 //       DataComputing::writeMatrix(res);
    ...
    void toUTDemo(){
        matrix m_matirx;
        DataComputing::readData(fileIn4,m_matirx);
        DataComputing::writeMatrix(m_matirx);
        std::cout<<std::endl;
        auto a = MatrixOperations::getMaxPosition(m_matirx,1);
/*         std::cout<< a.first<<" "<<a.second<<std::endl;
 */        MatrixOperations::matrixToUpperTriangleFormFullChoice(m_matirx);
        DataComputing::writeMatrix(m_matirx);
    }
...
int main() {
   /*  Matrix a{{2,-3,4},{1,0,-1}};
    Matrix b{{5,-3},{-1,0},{6,7}};
    std::cout<<a<<std::endl<<b<<std::endl;
    Matrix v1{{1,1,1}};
    Matrix v2{{2,2,2}};
    auto tmp = v1.transpose();
    std::cout<<tmp*v2<<std::endl;
    std::cout<<a.addToRow(0,1,-1) */;
  //  Lab1::toUTDemo();
  // Lab1::inverseMatirxDemo();
    Matrix a/* ({{10,5,3,0},{6,1,5,6},{2,-2,1,-2},{0,4,-1,2}},1E-10) */; 
    std::ifstream file("../tests/L1/test4.txt");
    file >> a;
    std::cout<<a.inverce();
    return 0;
}
